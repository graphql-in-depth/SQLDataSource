...
class SQLDataSource extends DataSource {
  ...
  cacheQuery(ttl = 5, query) {
    ...
    if (DEBUG && this.runCache[cacheKey]) console.log(`SQL cache use (${ttl}) [${cacheKey}]`);

    return this.runCache[cacheKey] || (this.runCache[cacheKey] = this.cache.get(cacheKey).then(entry => {
      if (DEBUG) console.log(`SQL cache ${entry ? 'get' : 'new'} (${ttl}) [${cacheKey}] ${query.toString()}`);
      if (entry) return Promise.resolve(JSON.parse(entry));

      return query.then(result => {
        if (DEBUG) console.log(`SQL cache set (${ttl}) [${cacheKey}] ${query.toString()}`);
